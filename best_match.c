...
int main(int argc, char* argv[]) {
    char alnfilename[MAXFILENAME];
    char cpsfilename[MAXFILENAME];
...
/*******************************************************************************************************/
    ...
    if(verbose) fprintf(stderr,"Reading CPS input, preliminary pass ...");
    max_sites=max_genes=0;
    while(fgets(buff,MAXBUFFLENGTH,cpsfile)) {
      	if(strlen(buff)<2) break;
        sscanf(buff,"%*s %*i %*c %i %i", &i, &j);
	...
	gene_idx[i]=gene_off[i]=0;
    }

    if(verbose) fprintf(stderr,"\nmax_genes = %i, max_sites = %i\nReading CPS input pass 1",max_genes,max_sites);
    cpsfile= fopen(cpsfilename,"r");
    while(fgets(buff,MAXBUFFLENGTH,cpsfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%s %*i %c %i %*i" , &chr1[0], &c, &i);
        gene_idx[i]++;
	gene_chr[i] = assign_code(chr1);
	gene_str[i] = c;
    }
    fclose(cpsfile);
...
/**********************************************************************************************/
    ...
    if(verbose) fprintf(stderr,"Reading ALN input pass 1");
    ...
    site_str   = (char*) malloc(sizeof(char)*(s+1));
    ...
    if(verbose) fprintf(stderr,", records = %i\nReading ALN input pass 2",s);
    alnfile = fopen(alnfilename,"r");
    while(fgets(buff,MAXBUFFLENGTH,alnfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%*s %*i %*c %s %i %c %*i %i %*c" , &chr2[0], &x, &c, &i);
	site_chr[site_idx[i]+site_off[i]] = assign_code(chr2);
        site_pos[site_idx[i]+site_off[i]] = x;
        site_str[site_idx[i]+site_off[i]] = c;
	...
	if(k>=0 && site_score[k]>0) {
	    fprintf(outfile,"%s\t%i\t%c\t%s\t%i\t%c\t",get_chr_name(gene_chr[i]),gene_pos[j],gene_str[i],get_chr_name(site_chr[k]),site_pos[k],site_str[k]);

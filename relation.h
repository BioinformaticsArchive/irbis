...
public:
    ...
    void get_from_bin_file(char*);
    void get_from_tab_file(char*);

    void get_from_bin_file(FILE*);
    void get_from_tab_file(FILE*);
...
void relation::get_from_tab_file(char *filename) {
    FILE *f;
    f = fopen(filename, "r");
    if(f==NULL) {
...
void relation::get_from_bin_file(char *filename) {
    FILE *f;
    f = fopen(filename, "rb");
    if(f==NULL) {
        if(verbose) fprintf(logfile,"Cannot load relation from '%s', exiting\n", filename);
...
void relation::get_from_tab_file(FILE *f) {
    keys_t left, right;
    keys_t i;
    index_t a;
    ...
    while(fgets(buff,MAXBUFFLENGTH,f)) {
        sscanf(buff,"%i %i",&left, &right);
        table[index[left]+count[left]]=right;
	count[left]++;
    }
    make_pass2();
    validate();
...
void relation::get_from_bin_file(FILE *f) {
    keys_t left, right;
    keys_t i;
    index_t a;
    char buff[MAXBUFFLENGTH];
